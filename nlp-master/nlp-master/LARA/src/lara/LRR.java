...
//		有必要initialize with他的training model吗？ 都是tripadvisor那应该要吧。。。。。
//		LRR model = new LRR(500, 1e-2, 5000, 1e-2, 2.0, "Data/Model/model_base_hotel.dat");//
		LRR model = new LRR(500, 1e-2, 5000, 1e-2, 2.0);
//		weights 叫做 m_alpha。 存在Vector4Review class里
//		model.EM_est 分为两步
//		1。EStep(Vector4Review vct) 推断出vector weights
//			infer_alpha 具体求weights的方法
//		2。MStep  我们应该不需要这步
		model.EM_est("Data/Vectors/vector_CHI_min2_a4.dat", 10, 1e-4);
		model.SavePrediction("Data/Results/not_pre.csv");
//		我们不需要这步
